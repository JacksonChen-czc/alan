...
import cn.chenzecheng.alan.account.entity.AccountBig;
import cn.chenzecheng.alan.account.service.IAccountBigService;
 ...
 */
@SpringBootTest
public class AccountBigDbInit {

    @Resource
    private IAccountBigService accountBigService;

    private final ExecutorService threadPool = new ThreadPoolExecutor(64, 128, 10L, TimeUnit.MILLISECONDS,
            new LinkedBlockingQueue<Runnable>(1024), new ThreadPoolExecutor.CallerRunsPolicy());

    @Test
    public void insert() {
        AccountBig account = createAccount();
        boolean save = accountBigService.save(account);
    ...
    public void batchInsertAccount() {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            threadPool.submit(() -> {
                ...
                int size = 10000;
                List<AccountBig> list = new ArrayList<>(size);
                for (int j = 0; j < size; j++) {
                    AccountBig account = createAccount();
                    list.add(account);
                }
                accountBigService.saveBatch(list);
    ...
    @NotNull
    private AccountBig createAccount() {
        AccountBig account = new AccountBig();
